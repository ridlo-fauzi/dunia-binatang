...
class TebakGambarGame(Screen):
    ...
    def __init__(self, **kwargs):
        super(TebakGambarGame, self).__init__(**kwargs)
        ...
        self.tree_image = ClickableImage(size_hint=(0.4, 0.2), pos_hint={'center_x': 0.5, 'center_y': 0.7},
                               source='assets/img/speakerOn.png')
        self.tree_image.bind(on_release=self.play_sound)
    ...
    def load_level_data(self, level):
        ...
        with open('assets/json/level_tebak_gambar.json', 'r') as f:
            level_data = json.load(f)

        if str(level) in level_data['levels']:
            level_info = level_data['levels'][str(level)]
            
            self.sound_file = SoundLoader.load(level_info['sound'])  
            self.correct_fruit = level_info['answer']
        else:
            print(f"Level {level} tidak ditemukan dalam file JSON.")


    def load_new_question(self):
        all_fruits = ["ayam", "buaya", "harimau", "jerapah", "kucing", "kuda", "rusa", "lumba-lumba", "tupai"]
    ...
    def load_stars_data(self):
        if os.path.exists('assets/json/level_tebak_gambar.json'):
            with open('assets/json/level_tebak_gambar.json', 'r') as file:
    ...
    def save_stars_data(self, level, stars):
        ...
        with open('assets/json/level_tebak_gambar.json', 'w') as file:
    ...
    def show_game_over_popup(self):
        ...
        self.lives_label.text = f"Nyawa: {self.lives}/{self.max_lives}"
    ...
    def mark_level_completed(self):
        ...
        level_screen_tebak_gambar = self.manager.get_screen('level_screen_tebak_gambar')
        level_screen_tebak_gambar.completed_levels[self.level - 1] = True  

        if self.level < len(level_screen_tebak_gambar.completed_levels):
            level_screen_tebak_gambar.completed_levels[self.level] = True


    def update_level_selection(self, level, is_correct, stars):
        """Memperbarui status level pada pemilihan level."""
        level_screen_tebak_gambar = self.manager.get_screen('level_screen_tebak_gambar')
        level_screen_tebak_gambar.on_answer_valid(level-1, is_correct, stars)  
    ...
    def show_complete_answer_popup(self, stars=0, correct_answer=False):
        ...
        # Menggunakan FloatLayout untuk memudahkan penempatan tombol
    ...
    def use_hint(self, instance):
        ...
        if self.hints_left > 0 and (self.hint_last_used is None or
            ...
            time_left = (self.hint_cooldown_time - (datetime.now() - self.hint_last_used).seconds) // 60
