...
class LevelScreenTebakGambar(Screen):
    def __init__(self, **kwargs):
        super(LevelScreenTebakGambar, self).__init__(**kwargs)
        ...
        for i in range(len(button_positions)):
            ...
            level_image = Image(source=level_image_source, size_hint=(None, None), size=(90, 90), pos_hint={'center_x': 0.5, 'top': 0.90})
    ...
    def load_levels_data(self):
        ...
        try:
            with open("assets/json/level_tebak_gambar.json", "r") as json_file:
    ...
    def start_game_for_level(self, level):
        ...
        tebak_gambar_game = self.manager.get_screen('tebak_gambar_game')
        tebak_gambar_game.start_game(level + 1)  
        self.manager.current = 'tebak_gambar_game' 
